...
function login($url,$data){
    $proxy = 'iphost';
    $credentials = "username:password";//gunakan credential disini
...
function grab_page($site){
    $proxy = 'iphost';
    $credentials = "username:password";//credential proxy
    $ch = curl_init();
...
function broadcast($id){
    switch ($id){
        case '2':
        $chatid = "xxxxxxxx";//userid telegram
        break;
...
// bagian close mainsfail
 $site = "http://10.9.97.226/api_cip/index.php/api/alarmMainsFail";
        ...
        $jumlah_mf = count($input['data'])-1;
        $arraydata = array();
        for ($j=0; $j <= $jumlah_mf ; $j++) { 
         $arraydata[] = $input['data'][$j]['SITE_ID'];
        }
        $filter_duplicate_data = array_unique($arraydata,SORT_REGULAR);

for ($i=0; $i <= $jumlah_data; $i++){
    if ($data_json["data"][$i][5] == "pln off mf"){
        //echo $data_json["data"][$i][1]; echo $data_json["data"][$i][3];
        $site_id = $data_json["data"][$i][3];
        $tiket_close = $data_json["data"][$i][1];
        $r = in_array($site_id, $filter_duplicate_data);
        var_dump($r);
            if ($r == false) {
                post_data("https://sumbagut.toti-telkomsel.com/Rtp/tiket/skip_rating","id=".$tiket_close);
            $alert_close .= $site_id ."\n";
